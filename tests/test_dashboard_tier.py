...
class TestFormatCampaignDf:
    ...
    def test_tier_populated_for_maintain(self):
        df = _format_campaign_df([self._row(cpd=30.0, deposits=5)], cpd_target=25.0)
    ...
    def test_multiple_rows_all_classified(self):
        rows = [
            self._row(campaign_name="A", cpd=10.0, deposits=5),   # SCALE
            self._row(campaign_name="B", cpd=30.0, deposits=5),   # MAINTAIN
            self._row(campaign_name="C", cpd=60.0, deposits=5),   # REDUCE
            self._row(campaign_name="D", cpd=10.0, deposits=0),   # PAUSED
        ]
